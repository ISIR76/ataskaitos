...
class EvaluationRequest(BaseModel):
    ...
    document_type: Literal["report", "article"] = Field(
        description="Type of document to evaluate"
    )
    evaluation_type: Literal["agent", "scoring"] = Field(
        default="scoring", description="Evaluation method to use"
    )
    ...
    store_result: bool = Field(
        default=False, description="Whether to persist evaluation result"
    )
...
class BaseEvaluationResponse(BaseModel):
    ...
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Evaluation metadata"
    )
...
class AgentEvaluationResponse(BaseEvaluationResponse):
    ...
    agent_output: Union[str, Dict[str, Any]] = Field(
...
class EvaluatorInfo(BaseModel):
    ...
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Additional evaluator metadata"
    )
...
class EvaluatorsListResponse(BaseModel):
    ...
    evaluators: Dict[str, List[EvaluatorInfo]] = Field(
